  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (attempt >= maxRetries) {
        throw new ChutesNetworkError(`Network error fetching ${url}: ${err.message}`, { cause: err });
      }
      const delay = baseDelay * 2 ** attempt + Math.random() * 1000;
      console.warn(`  [retry] ${url} → network error (${err.message}), retrying in ${Math.round(delay)}ms...`);
      await sleep(delay);
      continue;
    }
