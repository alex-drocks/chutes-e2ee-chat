...
import { ChutesE2EESecurityError, ChutesModelNotFoundError } from './errors.js';
import {
  DEFAULT_API_BASE,
  DEFAULT_MODELS_BASE,
  MODEL_MAP_TTL_MS,
...
export class ChutesDiscoveryManager {
  ...
  setAuth(apiKey) {
    this._authHeaders = { Authorization: `Bearer ${apiKey}` };
    // Discovery responses and nonces are scoped to the credential that fetched
    // them. Rotating credentials must not reuse another account's cached data.
    this.clearNonceCache();
    this.clearModelMap();
  }
  ...
  async resolveE2EEChuteId(model) {
    if (this._looksLikeUUID(model)) {
      throw new ChutesE2EESecurityError(
        'Raw chute IDs are not accepted for E2EE chat. Choose an advertised confidential-compute model name so metadata can be verified.',
      );
    }

    await this._maybeRefreshModelMap();
    let meta = this._modelMeta.get(model);

    if (!meta) {
      this._modelMapLoadedAt = 0;
      await this._maybeRefreshModelMap();
      meta = this._modelMeta.get(model);
    }

    if (!meta) throw new ChutesModelNotFoundError(model);
    if (!isE2EETextModelMeta(meta)) {
      throw new ChutesE2EESecurityError(
        `Model '${model}' is not advertised as a confidential-compute text model. Refusing to send encrypted chat payload.`,
      );
    }
  ...
  async _maybeRefreshModelMap() {
    ...
    let refresh;
    refresh = _fetchWithRetry(`${this._modelsBase}/v1/models`, {
        ...
        for (const entry of data.data || []) {
          if (entry.id && entry.chute_id) {
            ...
            newMeta.set(entry.id, {
              ...
              // Do not coerce strings such as false into a trusted E2EE
              // capability. Confidential-compute metadata is security-sensitive.
              confidentialCompute: entry.confidential_compute === true,
            });
          }
        }

        // A credential change or explicit cache clear may have superseded this
        // request while it was in flight. Never let the stale response restore
        // data that was deliberately invalidated.
        if (this._modelMapRefreshing !== refresh) return;
        this._modelMap = newMap;
        this._modelMeta = newMeta;
        this._modelMapLoadedAt = Date.now();
      })
      .finally(() => {
        if (this._modelMapRefreshing === refresh) {
          this._modelMapRefreshing = null;
        }
      });

    this._modelMapRefreshing = refresh;
    return refresh;
  }
  ...
  async getNonce(chuteId, _retries = 0) {
    ...
    if (!refresh) {
      let nextRefresh;
      nextRefresh = this._fetchInstances(chuteId)
        .then((discovery) => {
          const fresh = {
            instances: discovery.instances,
            expiresAt: discovery.nonceExpiresAt,
          };
          // clearNonceCache() can invalidate this request and allow a newer
          // refresh to start. Do not let the older request repopulate the cache.
          if (this._nonceRefreshes.get(chuteId) === nextRefresh) {
        ...
        .finally(() => {
          ...
          // refresh doesn't poison the map and block future retries. The
          // identity check preserves a newer refresh after invalidation.
          if (this._nonceRefreshes.get(chuteId) === nextRefresh) {
  ...
  async _fetchInstances(chuteId) {
    ...
    const { instances, rejectedCount } = normalizeE2EEInstances(rawInstances);

    if (rawInstances.length > 0 && instances.length === 0) {
      throw new ChutesE2EESecurityError(
        `Chutes returned ${rawInstances.length} E2EE instance(s), but none had valid public key and nonce material.`,
      );
    }

    if (rejectedCount > 0) {
      console.warn(`  [e2ee] Ignored ${rejectedCount} malformed E2EE instance record(s) for chute ${chuteId}`);
    }
...
function isE2EETextModelMeta(meta) {
  const inputModalities = Array.isArray(meta?.inputModalities) ? meta.inputModalities : ['text'];
  const outputModalities = Array.isArray(meta?.outputModalities) ? meta.outputModalities : ['text'];

  return Boolean(meta?.id) &&
    Boolean(meta?.chuteId) &&
    meta.confidentialCompute === true &&
    inputModalities.includes('text') &&
    outputModalities.includes('text');
}

function normalizeE2EEInstances(rawInstances) {
  const instances = [];
  let rejectedCount = 0;

  for (const inst of rawInstances) {
    const instanceId = normalizeHeaderValue(inst?.instance_id, 256);
    const e2ePubkey = normalizeE2EPubkey(inst?.e2e_pubkey);
    const nonces = Array.isArray(inst?.nonces)
      ? inst.nonces.map((nonce) => normalizeHeaderValue(nonce, 4096)).filter(Boolean)
      : [];

    if (!instanceId || !e2ePubkey || nonces.length === 0) {
      rejectedCount += 1;
      continue;
    }

    instances.push({ instanceId, e2ePubkey, nonces });
  }

  return { instances, rejectedCount };
}
...
function normalizeE2EPubkey(value) {
  const normalized = normalizeHeaderValue(value, 4096);
  if (!normalized) return '';

  try {
    return Buffer.from(normalized, 'base64').length === MLKEM_PK_SIZE ? normalized : '';
  } catch {
    return '';
  }
}
